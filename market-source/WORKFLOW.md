...
只维护A股。页面依次为原版双栏的价格与价值（左侧资产文字对比，右侧攻守与四项行动）、20项市场钟摆、关键数据与后续条件。页头仅名称和日期；不再有市场总结、周期/攻守等分数、温度计、美股、情绪周线、固定评分、日期台账、上周条件复盘和二层思维章节。
...
prepare绑定报告、源码和提示词指纹，并校对研究输入的日期、修订号和估值表。之后改动相关内容必须重新prepare和构建。archive在全部冲突检查通过后复制并验证哈希，最后更新目录/latest；不执行Git推送。浏览器检查正文顺序、已删除内容、估值日期、缺失值、文字判断、证据折叠、研究附件中的计算、钟摆筛选、链接和窄屏排版。
...
- markets.cn.valueAnalysis：统一日期估值表、五类资产的支持与反证、判断边界、后续条件、估值情景、口径冲突。cases.presentation保留名称、确定性、安全垫、应对文字；stance保留攻守结论与四项行动。默认不展开研究详情，不在正文铺开估值表或计算器。不保留hero、summary、score、signals、styleMap或defenseScore。
...
prepare从原始财务值、行业权重和同日估值重算关键比例与情景，结果和存储值不符即停止。文字判断由支持与反证共同解释，不假装是计算出的概率或分数。
