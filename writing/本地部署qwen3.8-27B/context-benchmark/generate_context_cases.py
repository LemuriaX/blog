...
from benchmark_console import console


def count_chat_tokens(base_url, messages, model, api_key, timeout, *, enable_thinking=False,
                      reasoning_effort="xhigh"):
    payload = chat_payload(messages, model, 1, 3407, enable_thinking=enable_thinking,
                           reasoning_effort=reasoning_effort)
    return count_prompt_tokens(base_url, api_key, payload, timeout)
...
def evaluate_size(
    ...
    api_key: str,
    timeout: int,
    *, enable_thinking: bool = False, reasoning_effort: str = "xhigh",
) -> tuple[int, str]:
...
def main() -> int:
    ...
    template_settings = dict(enable_thinking=args.enable_thinking, reasoning_effort=args.reasoning_effort)

    for case_index in range(1, args.cases + 1):
        console.emit("语料", f"{case_index}/{args.cases} | 寻找接近 {args.target_tokens:,} Token 的输入长度")
        ...
        empty_tokens = count_chat_tokens(args.base_url, [{"role": "user", "content": ""}], model, args.api_key,
                                         args.timeout, **template_settings)
        for needle in needles:
            ...
            if offset >= 0:
                prefix_count = count_chat_tokens(args.base_url, [{"role": "user", "content": content[:offset]}],
                                                model, args.api_key, args.timeout, **template_settings)
        ...
        console.emit("语料", f"{case_index}/{args.cases} 完成 | {actual_tokens:,} Token | 偏差 {difference:+d} | {json_path.name}")
