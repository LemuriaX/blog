...
def validate_runtime(status, log, stage):
    ...
    check("loaded_model_file", True, same_path, "llama_model_loader source path compared with preflight-hashed GGUF")
    ...
    check("context_per_sequence", stage["context"],
          last(r"\bn_ctx_per_seq\s*=\s*(\d+)", int), "llama.cpp startup log")
