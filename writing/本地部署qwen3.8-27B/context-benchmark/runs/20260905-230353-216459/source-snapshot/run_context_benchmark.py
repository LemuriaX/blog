...
from benchmark_console import Activity, RequestProgress, console, duration
...
def main():
    ...
    console.emit("题集", f"计划 {len(planned)} 次 | 本次执行 {len(pending)} 次 | Thinking 关闭")
    if args.resume:
        console.detail("恢复运行；进度条只统计本次执行，主表保留每题首次结果。")
    try:
        if not args.skip_warmup and pending:
            ...
            try:
                console.emit("预热", f"1 次长输入预热，最多输出 {warmup['max_tokens']} Token，不计入统计")
                started = time.monotonic()
                with Activity("预热请求正在执行"):
                ...
                console.emit("预热", f"完成 | 用时 {duration(time.monotonic() - started)}")
