...
def chat_payload(messages, model, max_tokens, seed, stream=True, *, enable_thinking=False,
                 reasoning_effort="xhigh", temperature=0.0, top_p=1.0, top_k=1,
...
def stream_chat_completion(base_url, payload, api_key, timeout, *, read_timeout=None, progress=None):
    ...
    def consume(data):
        ...
        for choice in choices:
            ...
            if content and result["ttfa_client_s"] is None:
                ...
                if progress:
                    progress("生成最终答案")
            elif reasoning and not result["reasoning"] and not result["answer"] and progress:
                progress("生成推理，尚未收到最终答案")
...
def execute_request(job, payload, journal, base_url, api_key, status, timeout, *, progress=None):
    ...
    try:
        ...
        if progress:
            progress("等待首个输出", input_tokens=counted)
        raw = stream_chat_completion(base_url, payload, api_key, timeout, progress=progress)
        if progress:
            progress("评分与保存结果")
