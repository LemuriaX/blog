...
def cli_help_supported(text, returncode):
    ...
    return returncode == 0 and all(flag in plain for flag in
                                  ("--model", "--max-seq-length", "--gpu-memory-mode", "--parallel"))


def read_logs(folder):
    return "\n".join(p.read_text(encoding="utf-8", errors="replace") for p in
                     (folder / "server.stdout.log", folder / "server.stderr.log") if p.exists())


def wait_ready(base_url, folder, managed, timeout):
    deadline = time.monotonic() + timeout
    api_key = os.getenv("UNSLOTH_API_KEY", "")
    last_error = "no ready model"
...
def run_stage(stage, config, root, args, models):
    ...
    for name in ("server.stdout.log", "server.stderr.log", "stage.json", "inference-status.json", "configuration-validation.json"):
...
def aggregate(root, plan):
    ...
    lines = ["# 测试运行摘要", "", "配置证据缺失会标为 unverified；测量结果和配置认证分开记录。", "",
...
def main():
    ...
    help_result = subprocess.run([str(args.unsloth_path), "run", "--help"], capture_output=True,
                                 text=True, encoding="utf-8", errors="replace", timeout=60,
                                 creationflags=subprocess.CREATE_NO_WINDOW if os.name == "nt" else 0)
    (root / "unsloth-run-help.txt").write_text(redact(help_result.stdout + help_result.stderr), encoding="utf-8")
    if not cli_help_supported(help_result.stdout + help_result.stderr, help_result.returncode):
        raise ValueError("Installed Unsloth CLI does not match the required launch interface")
    aggregate(root, plan)
    for stage in plan:
